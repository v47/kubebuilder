# multicluster-runtime/v1 Plugin

The `multicluster-runtime/v1` external plugin adds
...
Use `multicluster-runtime/v1` when you need to:
...
- Kubebuilder v4+
- Go 1.22+
- A working kubebuilder installation (`kubebuilder init` works)

## Installation

```bash
# Clone the kubebuilder repository (or use the standalone release)
git clone https://github.com/kubernetes-sigs/kubebuilder
cd kubebuilder/external-plugins/multicluster-runtime/v1
...
```
~/.config/kubebuilder/plugins/multicluster-runtime/v1/multicluster-runtime
```

## Project initialization

Chain `multicluster-runtime/v1` after `go/v4`:

```bash
kubebuilder init \
  --plugins go/v4,multicluster-runtime/v1 \
  --domain example.com \
...
```bash
kubebuilder init --plugins go/v4,multicluster-runtime/v1 \
...
The plugin generates a standard webhook file identical to `go/v4`'s output, with a
comment explaining the local vs. multicluster scope.
...
```bash
kubebuilder edit --plugins multicluster-runtime/v1 --provider namespace
```
...
This plugin is designed to run **after** `go/v4`. The plugin chain `go/v4,multicluster-runtime/v1`
means:

1. `go/v4` scaffolds the standard project structure
2. `multicluster-runtime/v1` rewrites `cmd/main.go` to use the multicluster manager

If `go/v4` is absent from the chain, a warning is printed to stderr.
