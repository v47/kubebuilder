...
// leaderElectionID is the marker comment inserted into every template; the
// template engine replaces it via the ProjectNameMixin and DomainMixin values.
const leaderElectionID = `{{ .ProjectName }}.{{ .Domain }}`
...
func main() {
	...
	provider := nsprovider.New(local, nsprovider.Options{})
...
func main() {
	...
	flag.BoolVar(&secureMetrics, "metrics-secure", true, "Serve the metrics endpoint securely via HTTPS.")
	flag.BoolVar(&enableHTTP2, "enable-http2", false, "Enable HTTP/2 for the metrics and webhook servers.")
	flag.StringVar(&kubeconfigSecretNamespace, "kubeconfig-secret-namespace", "default",
		"Namespace where Cluster API kubeconfig Secrets are stored.")
	...
	// The Cluster API provider reads kubeconfig Secrets created by Cluster API and
	// registers each managed cluster with the multicluster manager.
	// Requires sigs.k8s.io/cluster-api in your go.mod:
	//   go get sigs.k8s.io/cluster-api@latest
	provider, err := capiprovider.New(cfg, capiprovider.Options{
		SecretNamespace: kubeconfigSecretNamespace,
	})
	if err != nil {
		setupLog.Error(err, "unable to create cluster-api provider")
		os.Exit(1)
	}
...
func main() {
	...
	provider, err := fileprovider.New(fileprovider.Options{Dir: kubeconfigDir})
	if err != nil {
	...
	if err := provider.SetupWithManager(ctx, mgr); err != nil {
		setupLog.Error(err, "unable to setup file provider")
		os.Exit(1)
	}
